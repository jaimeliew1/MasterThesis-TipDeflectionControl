...
def estimateDeformation(M, y, offset=-4400000):
...
def plotDeformation(i):
    ...
    plt.legend(['TD sensor','RBM sensor', 'TD & RBM sensors', 'actual'])
    plt.show(); print()
    time.sleep(0.1)
...
#%% Optimise deformation function using RBM
def func(X):
    E = []
    for i in range(0, len(data)):
        ...
        RBM = -data.RBM1[i]*1000
        TD = data.TD1[i] - prebend[-1]
        est = estimateDeformation3(RBM, TD, offset=X) + prebend
        E.append(sum((est - data2[i,:])**2))
    return np.mean(E)
...
# Optimise deformation function using both RBM and TD
def func(X):
    E = []
    for i in range(0, len(data)):
        ...
        RBM = -data.RBM1[i]*1000
        TD = data.TD1[i] - prebend[-1]
        est = estimateDeformation(RBM, TD, offset=X) + prebend
        E.append(sum((est - data2[i,:])**2))
    return np.mean(E)
...
#%% Optimise deformation function using TD
def func(X):
    E = []
    for i in range(0, len(data)):
        ...
        RBM = -data.RBM1[i]*1000
        TD = data.TD1[i] - prebend[-1]
        est = estimateDeformation2(RBM, TD, offset=X) + prebend
        E.append(sum((est - data2[i,:])**2))
    return np.mean(E)
