...
def plot_bars_on_ax(ax, dlc_noipc, dlc, dlc2, c, key):
    ...
    labels = ['$A_r=1m$', '$A_r=2m$', '$A_r=3m$', '$A_r=4m$']
...
# bar graph
    ...
    ax.bar(WSP, Req_ol, width, label = 'CPC only', hatch='\\\\', fc='0.8', ec='0')
    ax.bar(WSP + width, Req_0, width, label = '$A_r=0m$', fc=colors[0], ec='0')
    for j, amp in enumerate(A):
...
def run(dlcs, SAVE=False):
    keys = ['RBMt']
    titles = ['Blade (torsion)']

    WSP = np.arange(4, 27, 4)
    fig, axes = plt.subplots(1, 1, sharex=True, sharey=True)
    plt.subplots_adjust(wspace=0.05, hspace=0.07)

    axes.set_xticks(WSP + 1)
    axes.set_xticklabels(WSP)
    axes.grid(True, axis='y')
    axes.set_axisbelow(True)
    axes.set_ylim(0, 1000)

    # normal shear
    dlc_noipc, dlc, dlc2 = dlcs['dlc11_0'], dlcs['dlc11_1'], dlcs['dlc11_3']

    for key, title, ax in zip(keys, titles, [axes]):
...
#    # inverse shear
#    dlc_noipc, dlc, dlc2 = dlcs['dlc15_0'], dlcs['dlc15_1'], dlcs['dlc15_2']
#    for key, title, ax in zip(keys, titles, axes[1, :]):
#        plot_bars_on_ax(ax, dlc_noipc, dlc, dlc2, 'ipc07', key)
#        ax.annotate(title + '\nInverse shear' , xy=(0.5, 0.95), xycoords='axes fraction',
#                size=10, ha='center', va='top',
#                bbox=dict(ec='w', fc='w', alpha=0.0))

# labels
    fig.text(-0.05, 0.55, '1 Hz Equivalent Bending Moment [kNm]', va='center', rotation='vertical')
    fig.text(0.5, 0.00, 'Wind Speed [m/s]', ha='center', rotation='horizontal')
    axes.legend(ncol=3,bbox_to_anchor=(1, 1.25))
...
if __name__ is '__main__':
    ...
    run(dlcs, SAVE='../Figures/Ch3_fatigue_bargraph_torsion_only_normalshear.png')
