...
#        pr1[a] = get_pr_data_from_sim(dlc2(controller=c, wsp=wsp, _amp=a)[0])
    ...
    # ipc07 pitch rate
    c = 'ipc07'
    pr2 = {0:get_pa_data_from_sim(dlc1(controller=c, wsp=wsp)[0])}
    for a in [1, 2, 3, 4]:
        pr2[a] = get_pa_data_from_sim(dlc2(controller=c, wsp=wsp, _amp=a)[0])
    ...
    #kde = kde_scipy(pr_noipc, x_, bandwidth=0.1)
    #plt.plot(x_, kde, '--', color='tab:orange', label='no IPC')
    plt.axvline(0, ls='--', c='tab:orange', label='no IPC')
    ...
    # labels
    ax.set_xlabel('Blade Pitch Angle Deviation $[^o]$')
