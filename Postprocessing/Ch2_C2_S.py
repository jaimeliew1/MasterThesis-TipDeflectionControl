...
def run(dlc=None, dlc_noipc=None, SAVE=None):
    ...
    for c in C:
        ...
        Mag.append(signal.bode(sys.T, w=f*(2*np.pi))[1])

    fig, ax = ControllerEvaluation.magplotSetup(F1p=0.16)
    ax.axhline(0, lw=1, c='0.7')
    ax.set_xlim(0.05, 1.5)

    ax.plot(f, Mag[0], '-.', label='Sensitivity Function')
    ax.plot(f, Mag[1], ':', label='Complementary Sensitivity Function')
